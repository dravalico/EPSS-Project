...
def init(threshold=0.7, storage_path='./datasets_new'):
    ...
    if not os.path.exists(complete_dataset_path):
        ...
        filter_by_threshold(df_dataset, threshold, storage_path, 'dataset_highest')
        filter_by_threshold(df_epss_v2, threshold, storage_path, 'epss_v2_highest')
        filter_by_threshold(df_epss_v3, threshold, storage_path, 'epss_v3_highest')
        print('Datasets with highest EPSS saved')


def filter_by_threshold(df, threshold, storage_path, name):
    highest_cve = set(df.groupby('cve').filter(lambda e: (e['epss'] > threshold).any())['cve'])
    df_highest = df[df['cve'].isin(highest_cve)]
    df_highest.to_csv(os.path.join(storage_path, name + '.csv'), index=False)
    df_highest.to_pickle(os.path.join(storage_path, name + '.pkl'))
